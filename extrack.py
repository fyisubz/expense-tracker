...
from models import add_expense, fetch_expenses, summary_by_category, monthly_summary
from analysis import visualise_by_category, visualise_monthly_summary, visualise_monthly_expenditure, export_expenses
...
def add_expense_action():
    ...
    if exceeded:
        overspent = total_spent - budget_limit
        messagebox.showerror( "Budget Alert",
...
def visualise_by_category_action():
    visualise_by_category()
...
tk.Button(root, text="Load Expenses", command=load_expenses).grid(row=3, column=2, pady=10)
...
def monthly_summary_action():
    summary = monthly_summary()
    if summary is None:
        messagebox.showinfo("Monthly Summary", "No data available")
    else:
        formatted_summary = "\n".join([f"{month}: {amt:.2f}" for month, amt in summary.items()])
        messagebox.showinfo("Monthly Summary", formatted_summary)

tk.Button(root, text="Monthly Expenditure", command=monthly_summary_action).grid(row=5, column=0, pady=10)
tk.Button(root, text="Monthly Visualiser", command=visualise_monthly_expenditure).grid(row=3, column=3, pady=10)
...
def budget_alert_action():
    ...
    if exceeded:
        ...
        messagebox.showerror(
            ...
            f"Total Spent: {total_spent:.2f}\n"
            f"Overspent by: {overspent_amount:.2f}"
        )
    else:
        messagebox.showinfo(
            "Budget Status",
...
tk.Button(root, text="Export Expenses", command=export_expenses).grid(row=6, column=3, pady=5)
root.mainloop()
